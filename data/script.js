...
function addChime(value = {enabled: true, hour: 8, minute: 30, weekdays: 62}) {
  ...
  const row = document.createElement('div'); row.className = 'chime';
  row.innerHTML = `<label><input class="enabled" type="checkbox"> 有効</label><input class="time" type="time"><label>月〜金 <input class="weekdays" type="checkbox"></label><button type="button">削除</button>`;
...
async function load() {
  ...
  $('#config').wifiSsid.value = config.wifiSsid || '';
  $('#config').ntp0.value = (config.ntpServers || [])[0] || 'ntp.nict.jp';
  ...
  $('#chimes').replaceChildren(); (config.chimes || []).filter(c => c.enabled).forEach(addChime);
...
$('#config').addEventListener('submit', async (event) => {
  ...
  const payload = {
    ...
    ledMode: Number(form.ledMode.value), solidColor: rgb(form.solidColor.value),
    digitColors: [0, 1, 2, 3].map(i => rgb(form[`digit${i}`].value)),
    animationSpeed: Number(form.animationSpeed.value), sleepEnabled: form.sleepEnabled.checked,
    sleepStartHour: Number(form.sleepStart.value.slice(0,2)), sleepStartMinute: Number(form.sleepStart.value.slice(3,5)),
    sleepEndHour: Number(form.sleepEnd.value.slice(0,2)), sleepEndMinute: Number(form.sleepEnd.value.slice(3,5)),
    chimes: [...document.querySelectorAll('.chime')].map(row => { const [h,m] = row.querySelector('.time').value.split(':').map(Number); return {enabled: row.querySelector('.enabled').checked, hour:h, minute:m, melodyId:0, weekdays: row.querySelector('.weekdays').checked ? 62 : 127}; }),
  };
  result.className = '';
  ...
  try {
    ...
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    result.className = 'ok';
    result.textContent = '保存しました。Wi-Fi接続とNTP同期を待機してください。';
